 ...
 * @version $Id: LC_Page_Admin_Contents_CustomerNews_Ex.php 22796 2013-05-02 09:11:36Z h_yoshimoto $
 */
class LC_Page_Admin_Contents_CustomerNews_Ex extends LC_Page_Admin_Ex {
    ...
    function init() {
        parent::init();
        $this->tpl_mainpage = 'contents/customer_news.tpl';
        $this->tpl_subno = 'customer_news';
        ...
        switch ($this->getMode()) {
            ...
            //----　表示順位移動
                if (strlen($news_id) > 0 && is_numeric($news_id) == true) {
                    ...
                    if ($term == 'up') {
                        $objDb->sfRankUp('dtb_customer_news', 'news_id', $news_id);
                    } else if ($term == 'down') {
                        $objDb->sfRankDown('dtb_customer_news', 'news_id', $news_id);
                    }
            ...
            //----　指定表示順位移動
                ...
                if (SC_Utils_Ex::sfIsInt($input_pos)) {
                    $objDb->sfMoveRank('dtb_customer_news', 'news_id', $news_id, $input_pos);
    ...
    function lfNewsInsert($arrPost,$member_id) {
        ...
        $table = 'dtb_customer_news';
        $sqlval = array();
        $news_id = $objQuery->nextVal('dtb_customer_news_news_id');
    ...
    function computeRankForDelete($news_id,$rank) {
        SC_Helper_DB_Ex::sfDeleteRankRecord('dtb_customer_news', 'news_id', $news_id);
    }
